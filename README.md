...
- **Agent-native** — Every operation is exposed through the CLI and its API, so a coding agent can drive ScriptHut end to end. `scripthut agent install` drops a Claude Code skill and a `/scripthut:debug` command into your project, `scripthut agent prompt` hands any agent a briefing of the available commands, and you can launch [Claude coding agents](docs/coding-agents.md) directly onto a git source from the CLI or browser.
