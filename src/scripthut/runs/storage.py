...
class RunStorageManager:
    ...
    def add_external_job(
        ...
        existing = run.get_item_by_backend_job_id(backend_job_id)
        if existing:
            # Update existing
            if state:
                try:
    ...
    def reconcile_external_jobs(
        ...
        """Mark non-terminal external jobs as completed if no longer active.

        Called after each poll cycle to reconcile stored external jobs against
        the current set of live backend job IDs.  Returns the number of items
        ...
        for run in runs_to_check.values():
            for item in run.items:
                if (
                    ...
                    and item.backend_job_id
                    and item.backend_job_id not in active_job_ids
                ):
