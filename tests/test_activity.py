...
class TestWindow:
    def test_window_matches_storage_retention(self):
        """A longer window could only render as permanently empty columns."""
        assert ACTIVITY_WINDOW_DAYS == RunStorageManager.RETENTION_DAYS

    def test_covers_exactly_the_trailing_window(self):
        grid = build_activity_grid([], today=TODAY, now=NOW)
...
class TestCpuHours:
    ...
    def test_runs_counted_once_per_day_regardless_of_task_count(self):
        ...
        grid = build_activity_grid([_run("r", items)], today=TODAY, now=NOW)
...
class TestLevels:
    def _grid_with(self, cpus_by_offset: dict[int, int]):
        ...
        return build_activity_grid([_run("r", items)], today=TODAY, now=NOW)
...
class TestGridLayout:
    ...
    def test_month_labels_mark_each_month_once(self):
        ...
        assert [name for _, name in labels] == ["Jul", "Aug"]
