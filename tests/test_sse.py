...
# -- QueueManager notify/wait tests ------------------------------------------


class TestNotifyQueue:
    """Tests for the QueueManager SSE notification mechanism."""

    def test_notify_creates_new_event(self) -> None:
        """notify_queue rotates the event, leaving a fresh one for next waiters."""
        mgr = _make_manager()
        mgr.notify_queue("q1")
        # After notify, there should be a fresh (unset) event
        assert "q1" in mgr._queue_events
        assert not mgr._queue_events["q1"].is_set()

    def test_notify_sets_old_event(self) -> None:
        """notify_queue sets the old event to wake waiters."""
        mgr = _make_manager()
        # Pre-create an event (simulates a waiter holding it)
        old_event = asyncio.Event()
        mgr._queue_events["q1"] = old_event
        mgr.notify_queue("q1")
        # Old event should be set
        assert old_event.is_set()
        # New event should not be the same object
        assert mgr._queue_events["q1"] is not old_event

    def test_notify_increments_version(self) -> None:
        """notify_queue increments the version counter."""
        mgr = _make_manager()
        assert mgr._queue_versions.get("q1", 0) == 0
        mgr.notify_queue("q1")
        assert mgr._queue_versions["q1"] == 1
        mgr.notify_queue("q1")
        assert mgr._queue_versions["q1"] == 2
