...
| `image` | no | string | `null` | Container image URI for this task, e.g. `ghcr.io/owner/repo:tag`. **Slurm:** `command` runs inside the image via `apptainer exec`; the image must already be on the backend (`scripthut image ensure`) or submission fails, and host paths beyond `$HOME` / `/tmp` / the working directory need the backend's `image_binds`. **AWS Batch:** overrides the backend's `default_image` when scripthut auto-registers a job definition; ignored when the backend has `job_definition` set (Batch locks the image to the registered definition; scripthut logs a warning on mismatch). Ignored by PBS. |
