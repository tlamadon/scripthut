...
If you want a coding agent (Claude Code, Codex, Cursor, Aider, etc.) to drive ScriptHut on your behalf, you don't need a custom plugin. ScriptHut exposes everything through the CLI; the only wiring needed is teaching the agent that the CLI exists.

## Claude Code: install the skill (recommended)

```bash
cd /path/to/your/project
scripthut agent install            # writes into ./.claude
scripthut agent install --user     # or into ~/.claude, for every project
```

This installs two things Claude Code picks up automatically:

- **A `scripthut` skill** (`.claude/skills/scripthut/SKILL.md`) — loads on demand when a task involves submitting or monitoring compute (on a cluster or the local backend), checking cluster status, tailing job logs, managing stacks, probing the result cache, or fetching task manifests. It's deliberately thin: its first instruction is to run `scripthut agent prompt` for the full, live, config-aware briefing, so nothing in it goes stale when you edit `scripthut.yaml`. Unlike piping the briefing into `CLAUDE.md`, it costs no context in sessions that never touch compute.
- **A `/scripthut:debug [run-id]` slash command** (`.claude/commands/scripthut/debug.md`) — walks the failure-diagnosis flow for a failed run: fetch statuses, pull stderr, classify the cause (stack not ready / resource limit / code bug), propose the fix.

Re-running `agent install` updates the files in place (they carry a `generated by` marker). Files you wrote by hand at those paths are never overwritten unless you pass `--force`.

## Other agents: pipe in the briefing

For agents without a skill mechanism, give them the output of one command:
